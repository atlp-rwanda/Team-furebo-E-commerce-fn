...
const NewPasswordForm = () => {
  ...
  const { successCondition, userInfo, error, pending } = useSelector((state) => state.newPassword);
  ...
  const handleSubmit = async (e) => {
    ...
    await newPasswordAction(authData, dispatch, params, () => {
      toast.success('Password was successfully reset, Please login', { theme: 'colored' });
      setTimeout(() => {
  ...
  return (
    <div className="authForm" data-testid="Newpasswordform">
      ...
      <div className="authFormRight">
        ...
        <form>
        <input name="newPassword" type="password" placeholder="New Password" value={authData.newPassword} onChange={handleChange} onKeyUp={handleValidationPassword} />
          {errors.newPassword && <p style={{color: "red", fontSize: "10px"}}>{errors.newPassword}</p>}
          <input name="confirmPassword" type="password" placeholder="Confirm Password" value={authData.confirmPassword} onChange={handleChange} onKeyUp={handleValidationPassword} />
          {errors.confirmPassword && <p style={{color: "red", fontSize: "10px"}}>{errors.confirmPassword}</p>}
          {errors.matchPassword && <p style={{color: "red", fontSize: "12px"}}>{errors.matchPassword}</p>}
          <span className='pending'>{pending ? 'loading...' : null}</span>
          {successCondition && <span className="successDisplay">{userInfo}</span>}
