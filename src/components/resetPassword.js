...
const ResetPasswordForm = () => {
  ...
  const { successCondition, userInfo, error, pending } = useSelector((state) => state.resetPassword);
  ...
  return (
    <div className="authForm" data-testid="Resetpasswordform">
      <div className="authFormLeft">
        ...
        <button onClick={()=> navigate ("/authentication")}>{isResetPassword ? 'Sign In' : 'Sign Up'}</button>
      </div>
      <div className="authFormRight">
        <h1>Reset Password</h1>
        <form>
          <input name="email" type="email" placeholder="Email" value={emailData.email} onChange={handleChange} onKeyUp={handleValidationEmail} />
